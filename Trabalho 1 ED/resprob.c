 ...
 * @date 18 May 2018
 * @brief Arquivo de implementação da função de resolução de equação.
...
void inf_to_postf(char* exp_infix, char* exp_postfix){
    ...
    for(i = 0, j = 0; i < strlen(exp_infix); i++){
        if(is_operand(exp_infix[i])){
            ...
            if(topo_pilha_char(itp_stack) == '('){
                desempilhar_pilha_char(itp_stack);
            }
        }
        else{
...
int resprob(){
    char expression[501], exp_postfix[501];
    double res;
